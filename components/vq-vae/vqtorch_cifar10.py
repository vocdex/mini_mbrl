"""VQ-VAE on CIFAR-10 dataset with wandb integration and lr scheduler using vqtorch.VectorQuant"""
import os
import random
...
from vqtorch.nn import VectorQuant  # Import the VectorQuant class from vqtorch
...
class Config:
    def __init__(self):
        ...
        # Model parameters
        self.latent_dim = 256
        self.num_embeddings = 1024# Size of the codebook (K)
        self.commitment_cost = 0.98  # Beta in paper
...
class VQVAE(nn.Module):
    ...
    Uses vqtorch.VectorQuant for improved quantization
    ...
    def __init__(self, config):
        ...
        # Vector Quantization using vqtorch.VectorQuant
        self.vq_layer = VectorQuant(
            feature_size=config.latent_dim,          # Feature dimension (D)
            num_codes=config.num_embeddings,         # Codebook size (K)
            beta=config.commitment_cost,             # Commitment cost (beta)
            kmeans_init=False,    # Use kmeans++ init
            sync_nu=config.sync_nu,                  # Codebook sync contribution
            replace_freq=config.replace_freq,        # Frequency to replace dead codes
            dim=1                                    # Dimension to quantize (1 for channels in BCHW format)
        )
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            indices: Indices of the nearest embeddings
        """
        z_e = self.encoder(x)
        
        # VectorQuant returns quantized vectors and a dictionary with various metrics
        z_q, vq_dict = self.vq_layer(z_e)
        
        # Extract relevant information from vq_dict
        vq_loss = vq_dict['loss']
        indices = vq_dict.get('q', None)  # Get indices from the dictionary
...
def calculate_perplexity(indices, num_embeddings):
    """
    Calculate perplexity given codebook indices.
    Perplexity is a measure of how many codes are being used effectively.
    """
    if indices is None:
        return torch.tensor(0.0)
        
    # Create a one-hot encoding of the indices
    # Flatten indices first to process batch as one
    indices_flat = indices.reshape(-1)
    
    # Count frequency of each code
    one_hot = F.one_hot(indices_flat, num_embeddings).float()
...
def evaluate(model, test_loader, config, global_step):
    ...
    # Calculate overall perplexity if we have indices
    if all_indices:
        all_indices = torch.cat(all_indices, dim=0)
        perplexity = calculate_perplexity(all_indices, config.num_embeddings)
    else:
    ...
    wandb.log(
        {
            ...
            "test/vq_loss": avg_vq_loss,
            "test/perplexity": perplexity.item(),
        }, 
        step=global_step
    )
    ...
    for epoch in range(config.num_epochs):
        ...
        for batch_idx, (data, _) in pbar:
            ...
            if batch_idx % config.log_interval == 0:
                # Calculate perplexity from batch indices if available
                if indices is not None:
                    batch_perplexity = calculate_perplexity(indices, config.num_embeddings)
                else:
                    batch_perplexity = torch.tensor(0.0)
        ...
        # Calculate overall perplexity for the epoch
        if all_indices:
            all_indices = torch.cat(all_indices, dim=0)
            epoch_perplexity = calculate_perplexity(all_indices, config.num_embeddings)
            perplexity_str = f", Perplexity: {epoch_perplexity.item():.2f}"
        else:
            perplexity_str = ""
...
def check_model_dimensions(config):
    ...
    with torch.no_grad():
        ...
        # Vector quantizer
        z_q, vq_dict = model.vq_layer(z_e)
        ...
        # Get perplexity if available in vq_dict, otherwise calculate it
        if 'perplexity' in vq_dict and vq_dict['perplexity'] is not None:
            perplexity = vq_dict['perplexity']
            print(f"VQ Perplexity from vq_dict: {perplexity:.2f}")
        elif 'q' in vq_dict:
            perplexity = calculate_perplexity(vq_dict['q'], config.num_embeddings)
            print(f"Calculated VQ Perplexity: {perplexity.item():.2f}")
        else:
            print("Perplexity calculation not available")
...
def main():
    ...
    cfg.save_dir = Path(f"./models_vqvae/{wandb.run.name}")
    ...
    # Initialize codebook with kmeans++ if enabled
    if cfg.kmeans_init:
        print("Warming up codebook (initializing with data samples)...")
        with torch.no_grad():
            for data, _ in tqdm(train_loader, desc="Initializing codebook"):
                if data.shape[0] < cfg.batch_size:
                    continue  # Skip incomplete batches
                data = data.to(cfg.device)
                z_e = model.encoder(data)
                # Run once to initialize with kmeans++
                model.vq_layer(z_e)
                break
