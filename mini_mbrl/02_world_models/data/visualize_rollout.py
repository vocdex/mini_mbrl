...
def visualize_entire_rollout_multiprocess(data_dir, save_as_gif=False):
    ...
    with Pool() as p:
        p.map(visualize_rollout_partial, [join(data_dir, file) for file in os.listdir(data_dir) if file.endswith(".npz")])
    

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--dir', type=str, help="Path to the .npz file folder", required=True)
    parser.add_argument('--save_gif', action='store_true', help="Save the rollout as a GIF")
