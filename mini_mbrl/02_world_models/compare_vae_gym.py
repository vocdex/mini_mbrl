...
def main(configs, record_gif=False):
    ...
    while not done:
        ...
        preprocessed_frame = (
            torch.tensor(preprocessed_frame, dtype=torch.float32)
            .unsqueeze(0)
            .to(device)
            / 255.0
        )
        ...
        with torch.no_grad():
            ...
            reconstructed_frame = (
                reconstructed_frame.squeeze(0).cpu().numpy().transpose((1, 2, 0))
            )
        ...
        combined_frame = np.hstack(
            (original_frame_display, reconstructed_frame_display)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--record_gif", action="store_true", help="Record the frames into a GIF"
    )
