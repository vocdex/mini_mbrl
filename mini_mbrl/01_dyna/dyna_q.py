    ...
    def learn(self, n_episodes):
        ...
        for _ in tqdm(range(n_episodes)):
            ...
            while not done:
                ...
                self.Q[state[0], state[1], action] += self.learning_rate * (
                    target - self.Q[state[0], state[1], action]
                )
...
class DynaQAgent(BaseAgent):
    ...
    def planning(self, n_planning_steps):
        for _ in range(n_planning_steps):
            state = (
                np.random.randint(0, self.env.observation_space.high[0]),
...
def compare_planning_steps(n_planning_steps=[0, 5, 50]):
    ...
    env = GridWorldEnv(
        ...
        width=10,
        start_position=(0, 0),
        goal_positions=[(9, 9), (9, 8)],
        trap_positions=[(5, 4), (5, 9)],
    )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--agent", type=str, default="dyna_q", help="sarsa, q_learning, dyna_q"
    )
    ...
    env = GridWorldEnv(
        height=args.height,
        width=args.width,
        start_position=eval(args.start_position),
        trap_positions=eval(args.trap_positions),
        goal_positions=eval(args.goal_positions),
    )
    if args.agent == "sarsa":
    ...
    if args.n_planning_steps != 0:
        plot_metrics(
            episode_rewards,
            episode_steps,
            agent_type=args.agent + "_" + str(args.n_planning_steps),
        )
